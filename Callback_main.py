...
def main(message, admins, account_name):
    ...
    one = types.InlineKeyboardButton(text='📊 Подбор запросов (WB)', callback_data='Tags_wbpartner')
    two = types.InlineKeyboardButton(text='📯 Подбор запросов (MP)', callback_data='Tags_mpstat')
    four = types.InlineKeyboardButton(text='💸 Рекламные ставки', callback_data='Advertising rates')
    five = types.InlineKeyboardButton(text='✏️ Изменение параметров аккаунта', callback_data='Changing account menu')
    six = types.InlineKeyboardButton(text='⚙️ Управление аккаунтами', callback_data='Account menu')
    ...
    markup_inline.add(two, four)
    if account_name != 'Не выбран':
        markup_inline.add(five)
